...
public class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        klineView = new NativeKLineView(this);
        ...
        if (chartContainer instanceof ViewGroup) {
            ViewGroup container = (ViewGroup) chartContainer;
    ...
    private List<KLineItem> calculateVolumeMAWithConfig(List<KLineItem> data, List<PeriodConfig> periodConfigs) {
        ...
        for (int index = 0; index < data.size(); index++) {
            KLineItem item = data.get(index).copy();
            List<MAItem> maVolumeList = new ArrayList<>(Collections.nCopies(2, null));
            for (PeriodConfig config : periodConfigs) {
                if (index < config.period - 1) {
                    maVolumeList.set(config.index, new MAItem(item.volume, String.valueOf(config.period)));
                } else {
                    double sum = 0;
                    for (int i = index - config.period + 1; i <= index; i++) {
                        sum += data.get(i).volume;
                    }
                    maVolumeList.set(config.index, new MAItem(sum / config.period, String.valueOf(config.period)));
                }
            }
    ...
    private boolean isMASelected() { return selectedMainIndicator == 1; }
    private boolean isBOLLSelected() { return selectedMainIndicator == 2; }
    private boolean isMACDSelected() { return selectedSubIndicator == 3; }
    private boolean isKDJSelected() { return selectedSubIndicator == 4; }
    private boolean isRSISelected() { return selectedSubIndicator == 5; }
    private boolean isWRSelected() { return selectedSubIndicator == 6; }
    ...
    private String fixRound(double value, int precision, boolean showSign, boolean showGrouping) {
    ...
    private static class KLineItem {
        ...
        JSONObject toJson() throws JSONException {
            ...
            for (Map<String, Object> item : selectedItemList) {
                ...
                selectedList.put(itemObj);
            }
            obj.put("selectedItemList", selectedList);

            if (maList != null) {
                JSONArray list = new JSONArray();
                for (MAItem item : maList) {
                    if (item == null) list.put(JSONObject.NULL);
                    else list.put(item.toJson());
                }
                obj.put("maList", list);
            }
            ...
            if (rsiList != null) {
                JSONArray list = new JSONArray();
                for (IndicatorItem item : rsiList) {
                    if (item == null) list.put(JSONObject.NULL);
                    else list.put(item.toJson());
                }
    ...
    private static class Theme {
        ...
        Theme(int backgroundColor, int titleColor, int detailColor, int textColor7724, int headerColor, int tabBarBackgroundColor,
              int backgroundColor9103, int backgroundColor9703, int backgroundColor9113, int backgroundColor9709, int backgroundColor9603,
              int backgroundColor9411, int backgroundColor9607, int backgroundColor9609, int backgroundColor9509, int backgroundColorBlue,
              int buttonColor, int borderColor, int backgroundOpacity, int increaseColor, int decreaseColor, int minuteLineColor, int gridColor,
              int separatorColor, int textColor) {
            this.backgroundColor = backgroundColor;
            this.titleColor = titleColor;
            this.detailColor = detailColor;
            this.textColor7724 = textColor7724;
            this.headerColor = headerColor;
            this.tabBarBackgroundColor = tabBarBackgroundColor;
            this.backgroundColor9103 = backgroundColor9103;
            this.backgroundColor9703 = backgroundColor9703;
            this.backgroundColor9113 = backgroundColor9113;
            this.backgroundColor9709 = backgroundColor9709;
            this.backgroundColor9603 = backgroundColor9603;
            this.backgroundColor9411 = backgroundColor9411;
            this.backgroundColor9607 = backgroundColor9607;
            this.backgroundColor9609 = backgroundColor9609;
            this.backgroundColor9509 = backgroundColor9509;
            ...
            this.minuteLineColor = minuteLineColor;
    ...
    private static class ThemeManager {
        static Theme currentTheme(boolean isDark) {
            if (isDark) {
                return new Theme(
                    colorStatic(0.07, 0.12, 0.19),
                    colorStatic(0.81, 0.83, 0.91),
                    colorStatic(0.43, 0.53, 0.66),
                    colorStatic(0.24, 0.33, 0.42),
                    colorStatic(0.09, 0.16, 0.25),
                    colorStatic(0.09, 0.16, 0.25),
                    colorStatic(0.03, 0.09, 0.14),
                    colorStatic(0.03, 0.09, 0.14),
                    colorStatic(0.13, 0.2, 0.29),
                    colorStatic(0.09, 0.16, 0.25),
                    colorStatic(0.03, 0.09, 0.14),
                    colorStatic(0.11, 0.17, 0.25),
                    colorStatic(0.07, 0.15, 0.23),
                    colorStatic(0.09, 0.15, 0.23),
                    colorStatic(0.09, 0.16, 0.25),
                    colorStatic(0.14, 0.51, 1),
                    colorStatic(0.14, 0.51, 1),
                    colorStatic(0.13, 0.2, 0.29),
                    colorStatic(0, 0, 0, 0.8),
                    colorStatic(0.0, 1.0, 0.53),
                    colorStatic(1.0, 0.4, 0.4),
                    colorStatic(0.14, 0.51, 1),
                    colorStatic(0.13, 0.2, 0.29),
                    colorStatic(0.13, 0.2, 0.29),
                    colorStatic(0.81, 0.83, 0.91)
            ...
            return new Theme(
                colorStatic(1, 1, 1),
                colorStatic(0.08, 0.09, 0.12),
                colorStatic(0.55, 0.62, 0.68),
                colorStatic(0.77, 0.81, 0.84),
                colorStatic(0.97, 0.97, 0.98),
                colorStatic(1, 1, 1),
                colorStatic(0.91, 0.92, 0.93),
                colorStatic(0.97, 0.97, 0.98),
                colorStatic(0.91, 0.92, 0.93),
                colorStatic(0.97, 0.97, 0.98),
                colorStatic(0.96, 0.97, 0.98),
                colorStatic(0.94, 0.95, 0.96),
                colorStatic(0.96, 0.97, 0.99),
                colorStatic(1, 1, 1),
                colorStatic(0.95, 0.97, 0.99),
                colorStatic(0, 0.4, 0.93),
                colorStatic(0, 0.4, 0.93),
                colorStatic(0.91, 0.92, 0.93),
                colorStatic(0, 0, 0, 0.5),
                colorStatic(0.0, 0.78, 0.32),
                colorStatic(1.0, 0.27, 0.27),
                colorStatic(0, 0.4, 0.93),
                colorStatic(0.91, 0.92, 0.93),
                colorStatic(0.91, 0.92, 0.93),
                colorStatic(0.08, 0.09, 0.12)
        ...
        private static int colorStatic(double r, double g, double b) {
            return colorStatic(r, g, b, 1);
        }
    ...
    private static class IndicatorTypes {
        ...
        static {
            main.put(1, new IndicatorType("MA", "ma"));
            main.put(2, new IndicatorType("BOLL", "boll"));
            main.put(0, new IndicatorType("NONE", "none"));

            sub.put(3, new IndicatorType("MACD", "macd"));
            sub.put(4, new IndicatorType("KDJ", "kdj"));
            sub.put(5, new IndicatorType("RSI", "rsi"));
            sub.put(6, new IndicatorType("WR", "wr"));
            sub.put(0, new IndicatorType("NONE", "none"));
        }
    }
}
